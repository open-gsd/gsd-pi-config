...
**Key Decisions:**
- Web-only shadcn restyle; desktop keeps legacy styling (ISO-01)
- Mist Sky palette (soft sky, radius 0, linear) — not logo cyan
- FND-03 primitive allowlist + isolation tests
- FormControls presentation adapters; no domain rewrite
- Human UAT for smoke/a11y; Playwright/axe deferred

**Issues Deferred:**
- Desktop shadcn migration
- Playwright / axe CI
- Nyquist validate-phase reconciliation for phases 1–4
- v2 polish (toasts, skeletons, tooltips, density, visual harness)

**Technical Debt:**
- Dual button languages in ConfigApp (web Button vs desktop uiClasses) — intentional
- MultiSelect chip-remove 24px accepted
- Source-level tests dominate over DOM E2E
